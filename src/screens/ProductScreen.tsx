  ...
  const addToCart = async () => {
    ...
    try {
      if (!cartItem) {
        ...
        // this check is not needed, but is a fail safe. 
        ...
        // if we wont go over 
        if (cartItem.quantityAvailable > itemQuantity) {
          addItemToCart(cartItem, 1)
          setItemQuantity((prev) => prev + 1)
        } else {
          console.log('we are going over')
        }
        ...
        >
          <View style={{ width: '100%' }}>
            <View style={{ display: 'flex', flexDirection: 'row', width: '100%', justifyContent: 'space-between' }}>
              ...
              {userToken && (
                inWishlist ? (
                  <TouchableOpacity style={{ marginRight: 5, marginTop: -4 }}
                    onPress={() => {
                      removeItemFromWishlist(data.id);
                      setInWishlist(!inWishlist);
                    }}>
                    {/* You can uncomment these if you prefer FontAwesome icons */}
                    ...
                    {/* <Image
                      ...
                      resizeMode="contain"
                    /> */}
                    <Ionicons name="heart" size={42} color={config.primaryColor} />
                  </TouchableOpacity>
                ) : (
                  <TouchableOpacity style={{ marginRight: 5, marginTop: -4 }}
                    onPress={() => {
                    ...
                    {/* <Image
                      ...
                      resizeMode="contain"
                    /> */}
                    <Ionicons name="heart-outline" size={42} color={config.primaryColor} />
                  </TouchableOpacity>
                )
              )}
            </View>
            ...
            <Text style={styles.smallPrice}>
              {
                ...
                `${data.priceRange.minVariantPrice.amount * 100 % 10 === 0 ? '0' : ''}` +
                ' USD'
              }
            ...
            {data.availableForSale && !userContinueAnyways ? (
              <View style={{
                ...
                // shadowOpacity: itemQuantity > 0 ? 0.12 : 0,
            ...
            ) : (!data.availableForSale && <Text>Out of stock</Text>)}
            ...
            <TouchableOpacity style={{ marginLeft: 0, marginTop: 20, }} onPress={() => { mainSheetRef.current?.snapToIndex(1); }}>
              <Text style={{ color: '#4B2D83', fontWeight: '600', fontSize: 20, paddingTop: data.description.length > 0 ? (0) : (25) }}>Discover More</Text>
            </TouchableOpacity>
          </View>
